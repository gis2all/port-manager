...
- `apps/desktop/src/lib/mockBackend.ts`
  - 浏览器预览用的模拟后端。
- `apps/desktop/src/lib/screenshotMode.ts`
  - dev-only 截图模式开关与固定快照常量。
  - 当前用于 `Ports` 首屏 1:1 复刻基线。
...
### dev 截图模式
- 当前 `import.meta.env.DEV` 下默认启用 `apps/desktop/src/lib/screenshotMode.ts`。
- 该模式会把前端 API 路由到 mock backend，即使在 Tauri 窗口中也不依赖真实本机端口数据。
- 主要目标是把 `Ports` 首屏固定在 `1536 x 1024` 参考图基线下做 1:1 复刻。
- 当前复刻范围仅覆盖 `Ports` 首屏，且文案、统计卡、Favorites、Activity、右侧详情均允许使用固定快照数据。

### Tauri 桌面模式
- 使用 `apps/desktop/src/lib/api.ts` 调用 `@tauri-apps/api/core` 的 `invoke`。
- 适合联调真实本机端口、进程和服务能力。
- 如果后续关闭截图模式，桌面端会恢复到真实数据链路。
