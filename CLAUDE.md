...
- 把端口和受管服务关联起来，支持从 UI 直接启动、停止、登记服务
...
桌面端右侧详情面板当前已经接入真实系统采集，不再使用前端硬编码猜测。
...
- 日志落地到 `data/logs/`
- 支持可选 `stop_command`
- 停止时优先执行 `stop_command`
- 停止失败或超时后回退到进程树终止
- 运行状态会根据 PID 存活情况自动修正 `starting/running/stopped/failed`
...
`crates/application` 负责业务编排和前后端 DTO：
...
- 桌面 dev 默认是实时数据
- 浏览器预览才是 mock 数据
- `screenshot mode` 已经移除，不再作为当前机制保留
...
- `D:/Code/port-manager/crates/adapters/runner/src/command_runner.rs`

当前会自动确保 `data/` 和 `data/logs/` 目录存在。
...
- `kill-port <port>`
- `favorite port <port>`
- `favorite service <uuid>`

输出格式：

- `scan-ports` 支持 `json` / `table`
...
- `D:/Code/port-manager/apps/desktop/public/port-manager.png`
...
- 真实进程详情已经接入，不能重新引入前端猜测型元数据
- 命令型服务当前依赖 `data/logs/` 记录日志，相关路径不要写散
- 项目识别当前已经接入桌面端和 CLI，后续描述能力时应以真实接线范围为准
...
- 主交付是 Tauri 桌面窗口
- 桌面 dev 默认走真实数据，不走 screenshot mode
- 数据库在 `data/port-manager.db`
- 命令型服务日志默认在 `data/logs/`
